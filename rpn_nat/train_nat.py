...
def train_rpn_llm(start_step=0, checkpoint_path=None, model_type="rdt", max_steps=80000, dataset_prefix="1-22_tens_comp_clean_tiered"):
    ...
    # NAT Batching Strategy
    B = 32
    grad_accum_steps = 4 
    # T is dynamic in NATDataset depending on the problem length, so total tokens varies per step.
    # We remove the static total_batch_size assertion.
    print(f"Micro batch size (Sequences): {B}")
    ...
    train_ds = NATDataset(train_dataset, max_length=128, mask_prob=0.8)
    val_ds = NATDataset(val_dataset, max_length=128, test_mode=True, mask_prob=0.8)
    
    train_sampler = BucketBatchSampler(train_ds, batch_size=B, shuffle=True)
    val_sampler = BucketBatchSampler(val_ds, batch_size=B, shuffle=False)
    
    train_loader = DataLoader(train_ds, batch_sampler=train_sampler, collate_fn=nat_collate_fn)
    val_loader = DataLoader(val_ds, batch_sampler=val_sampler, collate_fn=nat_collate_fn)
    ...
    for step in range(start_step, max_steps):
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device, dtype=torch.bfloat16):
                # We no longer need manual y masking here because NATDataset
                # already sets prompt positions to -100.
        ...
        t1 = time.time()
        dt = (t1 - t0)*1000 # ms
        tokens_per_sec = grad_accum_steps * B * x.size(1) / (t1 - t0)
        if (step+1) % 100 == 0:
        ...
        if (step+1) % 8000 == 0:
            ...
            torch.save(checkpoint, f'rpn_nat/models/{run_name}_{step+1}.pt')
            print(f"Model checkpoint saved to rpn_nat/models/{run_name}_{step+1}.pt")
