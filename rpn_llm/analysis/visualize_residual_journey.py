...
def visualize_journey(step):
    ...
    sim_to_start = np.zeros((T, num_states))
    sim_to_final = np.zeros((T, num_states))
    ...
    colors = plt.cm.tab20(np.linspace(0, 1, T))
    ...
    for t_idx in range(T):
        ...
        plt.plot(range(num_states), sim_to_start[t_idx, :], color=colors[t_idx], linestyle='-', linewidth=2.5, alpha=0.8, label=label_base)
        
        # Dotted line: 1 - Sim to Final
        plt.plot(range(num_states), 1.0 - sim_to_final[t_idx, :], color=colors[t_idx], linestyle=':', linewidth=2.5, alpha=0.8)

    plt.title(f"Residual Stream Journey (Step {step})\nSolid: Sim to Start | Dotted: 1 - Sim to Final", fontsize=16)
    ...
    out_path = f"rpn_llm/analysis/residual_journey_gated_{step}.png"
    plt.savefig(out_path)
    print(f"Saved {out_path}")
    plt.close()

if __name__ == "__main__":
    visualize_journey(8000)
    visualize_journey(80000)
