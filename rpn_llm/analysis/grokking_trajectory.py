...
def get_weight_metrics(model_path, prev_weights=None, final_weights=None, target_layer="transformer.h.0.attn.c_attn.weight", device='cpu'):
    ...
    norm = torch.linalg.vector_norm(w, ord=2).item()
...
def analyze_grokking(checkpoint_pattern, target_layer, run_fidelity=False, num_trials=20, benchmark_type="short", device='cpu'):
    ...
    if run_fidelity:
        ...
        if not os.path.exists(benchmark_path):
            ...
            test_samples = benchmark.get(benchmark_type, [])
            if not test_samples:
                print(f"Warning: Benchmark type {benchmark_type} not found. Using 'short'.")
                test_samples = benchmark["short"]
            print(f"Using {min(len(test_samples), num_trials)} {benchmark_type} trials per checkpoint for fidelity scoring.")
    
    prev_w = None
    layer_slug = target_layer.replace('.', '_')
    data_out = f"rpn_llm/analysis/grokking_data_{layer_slug}_{benchmark_type}.json"
    ...
    for i, p in enumerate(paths):
        ...
        # Fidelity analysis
        if run_fidelity:
            print(f"[{i+1}/{len(paths)}] Step {step}: Analyzing Fidelity...", end=" ", flush=True)
            subset = test_samples[:num_trials]
            score = run_fidelity_test(p, subset, device=device, verbose=False)
            fidelity_scores.append(score)
            print(f"Score: {score:.1f}%")
        else:
            print(f"[{i+1}/{len(paths)}] Step {step}: Weight drift only.")
    ...
    if run_fidelity:
        ...
        line4 = ax2.plot(steps, fidelity_scores, label=f'Fidelity ({benchmark_type})', color='red', linewidth=3)
    ...
    plt.title(f'Grokking Trajectory ({benchmark_type}): {target_layer}')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    
    out_name = f"rpn_llm/analysis/grokking_plot_{layer_slug}_{benchmark_type}.png"
    plt.savefig(out_name)
    print(f"Plot saved to {out_name}")
    print(f"Final data saved to {data_out}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--layer", type=str, default="transformer.h.attn.c_attn.weight")
    parser.add_argument("--fidelity", action="store_true", help="Run behavioral fidelity test")
    parser.add_argument("--trials", type=int, default=20, help="Number of trials for fidelity test")
    parser.add_argument("--benchmark", type=str, default="short", choices=["short", "long"], help="Benchmark type")
    parser.add_argument("--device", type=str, default="cpu")
    args = parser.parse_args()
    
    analyze_grokking(args.pattern, args.layer, args.fidelity, args.trials, args.benchmark, args.device)
