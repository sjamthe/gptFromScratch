...
print(f"{'N2 Len':<8} | {'Total Digits':<15} | {'Prompt Len':<15} | {'Accuracy':<10}")
print("-" * 60)

n1_len = 21
for n2_len in range(4, 23):
    prompt_len = 1 + n1_len + 1 + n2_len + 2 # [BOS] + N1 + SPACE + N2 + +?
    acc = test_combination(n1_len, n2_len, num_trials=20)
    print(f"{n2_len:<8} | {n1_len + n2_len:<15} | {prompt_len:<15} | {acc:>9.1f}%")
    if acc < 90.0: # A bit of buffer for the 95% target
        break
